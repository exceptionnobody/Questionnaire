...
const Filters = (props) => {
  const {items, filtraQuestionario, setShowCompila, loggedIn, setRicaricaUtenti2, bloccaFiltri} = props;
  return (
    <ListGroup as="div" variant="flush">
        {
          items.map((t,i) => {
            const valid= i
            return (
              <ListGroup.Item disabled={bloccaFiltri} as="a" key={t.qid} onClick={()=>{
